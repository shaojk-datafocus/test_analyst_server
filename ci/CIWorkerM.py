...
class CIWorker():
    ...
    def working(self):
        while not self.quit:
            ...
            if info:
                print(info)
                p = Process(target=getattr(self, info["command"]),args=info["args"],kwargs=info["kwargs"])
                p.start()
    ...
    def __del__(self):
        self.close()
        print("Dismiss a worker assigned by %s:%d"%self.addr)
...
if __name__ == '__main__':
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            c, addr = s.accept()
            p = Process(target=assign_worker,args=(c,addr,option.host,option.root,option.username,option.compat2))
            p.start()
